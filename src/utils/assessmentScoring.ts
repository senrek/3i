...
export const calculateCategoryScore = (
  ...
  categoryQuestions.forEach(question => {
    ...
    if (question.category === 'aptitude') {
      ...
      // For multiple selection interest questions (100-103)
      else if (question.id.startsWith('int_') && Number(question.id.replace(/\D/g, '')) >= 100) {
        const selections = answer.split(',');
        if (selections.length > 3) {
          // If more than 3 options selected, only count top 3
          selections.slice(0, 3).forEach(selection => {
            if (selection !== 'F') score += 2; // 2 points for each valid selection
          });
        } else {
        ...
        totalPossibleScore += 6; // Maximum possible score for 3 selections
      }
